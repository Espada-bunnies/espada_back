...
class ActionService(PublicationService):
    ...
    def logic_for_like_dislike_or_sharing(self, request, *args, **kwargs):
        ...
        obj = self._get_or_none(
            self.rating_queryset, relation=obj_id, rated_user=user_id
        )
        if action == "sharing":
            self.sharing_queryset.objects.create(
                relation_id=obj_id, shared_user_id=user_id
            )
        elif action == "like":
            if self._check_already_exists(obj):
                pass
            else:
                self.rating_queryset.objects.create(
                    relation_id=obj_id, rated_user_id=user_id, positive=False
                )
        elif action == "dislike":
